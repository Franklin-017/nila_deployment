...
function setup() {
  ...
  World.add(world, [ground, leftWall, rightWall, mConstraint]);
}
...
function mouseMovedEvent() {
  ...
  if (Matter.Query.point(bodiesList, position).length) {
    mouseOverBodies = true;
  } else {
    mouseOverBodies = false;
  }
}

function getMouseOverFlag() {
    return mouseOverBodies;
}
